...
- 提供播放/暂停/速度控制
- 无需用户调节参数，被动观看即可
- 重点展示：随时间变化的过程、状态转移、波形传播等

代码结构要求：
- 使用requestAnimationFrame实现动画循环
- 预设计算关键帧数据，避免实时复杂计算
- 关键帧数据必须完整：至少15-20个关键帧，覆盖完整周期
- 提供重置按钮回到初始状态

【强制要求 - 关键帧数据】
```javascript
// 必须提供完整的关键帧数据，禁止只写第0帧
const keyFrames = {
    0: { /* 状态1 */ },
    1: { /* 状态2 */ },
    // ... 至少15-20个关键帧，覆盖完整动画周期
    19: { /* 最终状态，与状态1衔接形成循环 */ }
};
const totalFrames = Object.keys(keyFrames).length;
```""",
...
代码结构要求：
- 滑块使用 oninput 事件（不是 onchange），确保拖动时实时更新
- 按钮使用 onclick 事件绑定
- 实时计算和渲染（性能优化）
- 清晰的参数-结果对应关系
- 所有绘制函数必须完整实现，禁止留空或写注释占位
...
SIMULATION_CODE_CONSTRAINTS = """【仿真代码强制约束 - 必须遵守】

### 1. 物理计算必须完整实现
- 所有物理公式必须写成可执行的JavaScript代码
- 禁止出现"此处省略具体计算"、"根据实际需求实现"等注释
- 禁止使用空的函数体或占位符
- 每个函数必须有实际可运行的代码逻辑

### 2. 关键帧数据必须完整
- 动画类型仿真：必须提供15-20个完整的关键帧数据
- 关键帧必须覆盖完整的动画周期
- 禁止使用注释代替实际数据：// 添加更多关键帧数据 ❌
- 必须确保动画循环时首尾衔接自然

### 3. 绘制函数必须完整
- draw()、drawFrame()、render()等绘制函数必须完整实现
- 必须包含实际的Canvas绘制代码（ctx.beginPath, ctx.moveTo, ctx.lineTo等）
- 禁止只写ctx.clearRect()然后留空
- 必须根据物理状态绘制对应的可视化效果
